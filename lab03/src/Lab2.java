...
public class Lab2 extends javax.swing.JFrame
{
	...
	void JButtonAllSwimmers_actionPerformed(java.awt.event.ActionEvent event)
	{
		...
		Iterator<Swimmer> it = slist.getVector().iterator();
		while(it.hasNext())
		{
	...
	void JButtonAllBackward_actionPerformed(java.awt.event.ActionEvent event)
	{
		...
		Iterator<Swimmer> rIt = IF.getIterator(vectorlist, StyleType.Reverse);
		while(rIt.hasNext())
		{
			Swimmer swimmer= rIt.next();
	...
	void JButtonAllAbove12_actionPerformed(java.awt.event.ActionEvent event)
	{
		Vector vectorlist = slist.getVector();
		IteratorFactory IF = new IteratorFactory();
		Iterator<Swimmer> aIt = IF.getIterator(vectorlist, StyleType.Above12);
		Iterator<Swimmer> paIt = IF.getIterator(vectorlist, StyleType.FunctorAbove12, p  -> p.getAge() > 12);
		
		/*while(aIt.hasNext())
		{
			Swimmer swimmer= aIt.next();
			JTextArea3.append(swimmer.getFname()+" "+swimmer.getLname()+ " \n");
		}*/
		
		while(paIt.hasNext())
		{
			Swimmer swimmer= paIt.next();
			JTextArea3.append(swimmer.getFname()+" "+swimmer.getLname()+ " \n");
		}
